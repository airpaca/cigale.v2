...
function Datablock(name, element, activated, bs_col_size, function_to_launch, dict_args) {
    ...
    */
    ...
    this.create = function() { 
        var html = '<div class="col-md-'+bs_col_size+' bg-color-white datablock '+this.name+'" id="'+this.name+'"></div>';
        $(html).appendTo(this.element);    
    };
    
    this.create_hidden = function() {
        $('<div class="col-md-'+bs_col_size+' bg-color-white datablock d-none" id="'+this.name+'"></div>').appendTo(this.element);    
    };
    ...
    this.createmanager = function(manager) {
        ...
        html += '<div class="col-md-' + bs_col_size + ' bg-color-white datablock" id="' + this.name + '">';
        ...
        for (var theme in manager){
            ...
            for (var layer in manager[theme].layers){
                ...
                if (this.layers_manager == true){
                    ...
                    if (app.blocks.mapview.layers[layer].visible == true) {var hide="";} else {var hide="invisible invisible-size";};
    ...
    this.createmap = function(dict_args){
        ...
        if ( (typeof dict_args["attributions"] != "undefined") && (dict_args["attributions"] == false) ) {
            ...
            map.attributionControl.setPrefix('Réalisé par AtmoSud et le GERES. <a target="_blank" href="https://leafletjs.com/">Leaflet</a>, fonds de carte <a target="_blank" href="https://www.esri.com/">&copy; Esri</a> & <a target="_blank" href="https://www.geoportail.gouv.fr/">&copy; Geoportail France</a>');
        };
        ...
        if ( (typeof dict_args.legend != "undefined") && (dict_args.legend == true) ){
            ...
            this.map.legend.onAdd = function (map) {
                ...
                div.innerHTML = '<button type="button" onclick="app.blocks.mapview.hide_show_legend();" class="close" aria-label="Close"><span aria-hidden="true">&times;</span></button>';
                return div;
            };
            ...
            this.map.legend_hidden.onAdd = function (map) {
                ...
                div.innerHTML = '<button type="button" onclick="app.blocks.mapview.hide_show_legend();" class="close" aria-label="Close"><span aria-hidden="true"><div style="font-size:12px;">Légende&nbsp;&nbsp;&loz;</div></span></button>';               
                return div;
            };
       ...
       this.printer = L.easyPrint({
        ...
        this.print_map = function(){
            ...
            // $("#map-block").removeClass("col-md-8");
            // $("#map-block").addClass("col-md-12"); 
            // app.blocks.mapview.map.invalidateSize();
            
            // this.printer.printMap('CurrentSize', 'cigale.map');
            
            // $("#map-block").removeClass("col-md-12");
            // $("#map-block").addClass("col-md-8");  
    ...
    this.activate_report_col = function(){
        ...
        if (solution == 1){
            ...
            if ($("#chk-stats-gen")[0].checked == true){
                ...
                $("#map-block").removeClass("col-md-6");
                $("#map-block").addClass("col-md-8"); 
    ...
    modalRatios = function(){
		...
		$.ajax({
			type: "GET", 
			url: "api.php/ratios_mob_defaut.php",
			dataType: 'json',  
    ...
    rapport_geres = function(){
        ...
        $.ajax({
            ...
            success: function(response,textStatus,jqXHR){               
                ...
                <p><b>Le gisement mobilisable à dire d’experts, hors STEP se répartit de la manière suivante</b></p>\
                "+create_html_table(gisements_graphs, ['groupe_rom','mob_t','mob_mwh'], ['groupe_rom','mob_t','mob_mwh'])+"\
                </br>");
