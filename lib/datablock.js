...
/* 		// Si bouton
    ...
    this.createmanager = function(manager) {
        ...
        for (var theme in manager){
            ...
            if ( (typeof manager[theme].info == "undefined") || (manager[theme].info != false) ){
                // html += '<button type="button" id="ButtonTheme'+manager[theme].id+'" class="list-group-item list-group-item-action rounded-0 dropdown-toggle ' + actif + '" onclick="app.blocks.left_layers.managethemes('+manager[theme].id+');">'+theme+'<a href="'+manager[theme].methodo+'" class="badge badge-pill ' + badge_class + '" id="ThemeMethodo'+manager[theme].id+'">i</a></button>';
                html += '<button type="button" id="ButtonTheme'+manager[theme].id+'" class="list-group-item list-group-item-action rounded-0 dropdown-toggle ' + actif + '" data-toggle="tooltip" data-placement="right" data-html="true" title="'+manager[theme].desc+'" onclick="app.blocks.left_layers.managethemes('+manager[theme].id+');">'+theme+'<a target="_blank" href="'+manager[theme].methodo+'" class="badge badge-pill ' + badge_class + '" id="ThemeMethodo'+manager[theme].id+'">i</a></button>';
            } else {
                ...
                html += '<button type="button" id="ButtonTheme'+manager[theme].id+'" class="list-group-item list-group-item-action rounded-0 dropdown-toggle ' + actif + ' " data-toggle="tooltip" data-placement="right" data-html="true" title="'+manager[theme].desc+'" onclick="app.blocks.left_layers.managethemes('+manager[theme].id+');">'+theme+'</button>';    
            };
                ...
                if (this.layers_manager == true){
                    ...
                    if (app.blocks.mapview.layers[layer].multi_style != false){
                        ...
                        for (var is in app.blocks.mapview.layers[layer].multi_style){
                            ...
                            html += '      <label class="btn btn-info '+active+' btn-outline-info btn-xs btn-unites" id="styles-couches" onclick=\'app.blocks.mapview.change_style("'+layer+'", "'+is+'")\'>';
					...
					if (app.blocks.mapview.layers[layer].typename == "Consultation sur un autre site") {
						// Si lien vers autre site bouton spécial
						// html += '<a href="https://projet-methanisation.grdf.fr/tester-mon-potentiel/carte-de-zonage-indicative"><img class="external-site-layer-link" src="img/icones/feather/92px.2px.clearblue/at-sign.svg" onmouseover="this.src=\'img/icones/feather/92px.2px.blue/at-sign.svg\';"  onmouseout="this.src=\'img/icones/feather/92px.2px.clearblue/at-sign.svg\';"></a>';
						html += '<a href="'+app.blocks.mapview.layers[layer].link+'" target="_blank"><img class="external-site-layer-link" src="img/icones/feather/92px.2px.clearblue/at-sign.svg" onmouseover="this.src=\'img/icones/feather/92px.2px.blue/at-sign.svg\';"  onmouseout="this.src=\'img/icones/feather/92px.2px.clearblue/at-sign.svg\';"></a>';
                    ...
                    if (app.blocks.mapview.layers[layer].fonction_desc != false){
                        html += '       <div><a href="#" class="badge btn-outline-info function-spe" onclick="'+app.blocks.mapview.layers[layer].fonction+'">'+app.blocks.mapview.layers[layer].fonction_desc+'</a></div>';
                    };   
    ...
    this.change_style = function(main_layer, status){ 
		...
		// app.blocks.mapview.layers[main_layer].style_orig.jenks_calcules = calc_jenks(
			// response, 
			// app.blocks.mapview.layers[main_layer].style_dict.jenks.field, 
			// app.blocks.mapview.layers[main_layer].style_dict.jenks.njenks, 
			// app.blocks.mapview.layers[main_layer].style_dict.jenks.colorscale
		// );
        ...
        function change_each_layer(the_layer){
            app.blocks.mapview.layers[the_layer].layer.eachLayer(function (layer) { 
                ...
                layer.setStyle({  
					// FONCTIONS QUI FONCTIONNENT AVEC DES CLASSES PREDEFINIES
					// ------
                    ...
                    fillColor: find_jenks_color(
						app.blocks.mapview.layers[main_layer].multi_style[status].jenks_calcules, 
						layer.feature.properties[app.blocks.mapview.layers[main_layer].multi_style[status].style.jenks.field]
					),
                ...
                // app.blocks.mapview.layers[the_layer]["legend"]["html"] = app.server.getLegendWFS(
                    ...
                    // app.blocks.mapview.layers[the_layer].multi_style[status].style // FIXME: Faut renvoyer la bon dict avec les jenks calcules
                // ); 
		...
		if (app.blocks.mapview.layers[main_layer].multi_style != "undefined") {
			var the_suffix = "</br>(" + app.blocks.mapview.layers[main_layer].multi_style[app.blocks.mapview.layers[main_layer].active_style].name + ")";
		} else {
			var the_suffix = "";
		};
		
		app.blocks.mapview.removeToLegend(app.blocks.mapview.layers[main_layer].typename.split(":")[1]+app.blocks.mapview.layers[main_layer].id_prefix); 
		app.blocks.mapview.layers[main_layer]["legend"]["html"] = app.server.getLegendWFS(
			app.blocks.mapview.layers[main_layer].typename, 
			app.blocks.mapview.layers[main_layer].typename.split(":")[1]+app.blocks.mapview.layers[main_layer].id_prefix, 
			app.blocks.mapview.layers[main_layer].name + the_suffix, 
			app.blocks.mapview.layers[main_layer].legend_carriage, 
			app.blocks.mapview.layers[main_layer].multi_style[status].style // FIXME: Faut renvoyer la bon dict avec les jenks calcules
		); 

		if (app.blocks.mapview.layers[main_layer].layer.options.minZoom > app.blocks.mapview.map.getZoom() || app.blocks.mapview.map.getZoom() > app.blocks.mapview.layers[main_layer].layer.options.maxZoom) {
		} else {
			if (app.blocks.mapview.layers[main_layer].actif == true){
				app.blocks.mapview.addToLegend(app.blocks.mapview.layers[main_layer]["legend"].html);
			};
		};
                            ...
                            if (typeof app.manager[app.blocks.mapview.layers[layer].theme] !== 'undefined') {
                                ...
                                if ((app.manager[app.blocks.mapview.layers[layer].theme].id == id) && (app.blocks.mapview.layers[layer].actif == false) && (app.manager[app.blocks.mapview.layers[layer].theme].default_layers.indexOf(app.blocks.mapview.layers[layer].name) > -1) ){
                                    this.addLayer(layer);
                                        ...
                                        if (    (this.layers_manager == true) && (app.blocks.mapview.layers[layer].typename != "En cours de construction") && (app.blocks.mapview.layers[layer].typename != "Consultation sur un autre site")  ) {
    ...
    this.removeLayer = function(layer, invisible){
        if ( (app.blocks.mapview.layers[layer].typename != "En cours de construction") && (app.blocks.mapview.layers[layer].typename != "Consultation sur un autre site") ) {
    ...
    this.createmap = function(dict_args){
        ...
        if ( (dict_args != null) && (dict_args.draw == true) ) {
			...
			L.drawLocal.edit.toolbar.buttons.edit = 'Modifier la zone existante';
			L.drawLocal.edit.toolbar.buttons.editDisabled = 'Aucune zone à modifier';
			L.drawLocal.edit.toolbar.buttons.remove = 'Supprimer la zone existante';
			L.drawLocal.edit.toolbar.buttons.removeDisabled = 'Aucune zone à supprimer';
			
			L.drawLocal.draw.handlers.circle.tooltip.start = 'Cliquer puis laisser appuyé pour définir une zone';
			L.drawLocal.edit.handlers.remove.tooltip.text = 'Cliquer sur la zone puis "Enregistrer" pour la supprimer';
			...
			L.drawLocal.edit.handlers.edit.tooltip.text = 'Utiliser les poignées blanche sur le cercle pour déplacer la zone ou modifier le rayon';
			L.drawLocal.edit.handlers.edit.tooltip.subtext = 'Appuyer sur "Annuler" pour revenir en arrière';
            
			L.drawLocal.draw.handlers.simpleshape.tooltip.end = 'Relacher la souris pour pour créer la zone';
            ...
            if (app.manager[theme].actif == true){
                ...
                for (var layer in app.manager[theme].layers){ // Uniquement pour les layers du theme actif
                    ...
                    if (app.blocks.mapview.layers[layer].layer.options.minZoom > zoom_level || zoom_level > app.blocks.mapview.layers[layer].layer.options.maxZoom) {
                        ...
                        // if ( ($("#chk-"+app.blocks.mapview.layers[layer].id)[0].checked == true) && (app.blocks.mapview.layers[layer].actif == false) ){
                        if ($("#chk-"+app.blocks.mapview.layers[layer].id)[0].checked == true){
							if (app.blocks.mapview.layers[layer].actif == false) {
                        ...
                        if (app.blocks.mapview.layers[layer].actif == true) {
							if ($("#chk-"+app.blocks.mapview.layers[layer].id)[0].checked == false) {
								$("#chk-"+app.blocks.mapview.layers[layer].id)[0].checked = true;
							}; 
    ...
    modalRatios = function(){
		...
		$.ajax({
			...
			success: function(response,textStatus,jqXHR){
                ...
                $('#urlModalTitle').html('Modification des ratios MOB<a target="_blank" href="https://tmp.atmosud.org/doc/geres_methanisation_doc.pdf" class="badge badge-pill badge-info-collapsed">i</a>');
                ...
                for (i in response) {        
                    ...
                    html += '</div>';
                    html += '<input id="mob-param-'+response[i].id_param+'" type="text" class="form-control" aria-label="Small" aria-describedby="inputGroup-sizing-sm" value="'+response[i].ratio_mobilisation_pct+'">';
...
/*     zoom_territoire = function(val, token){
    ...
    rapport_geres = function(){
		...
		// Si ratios définis par l'utilisateur on les envoie
		if (typeof  app.ratios != "undefined") {
			var ratios = app.ratios;
		} else {
			var ratios = [];
		};
        ...
        $.ajax({
            ...
            success: function(response,textStatus,jqXHR){               
                ...
                var cql_polygon_filter = response[0][0].st_astext;
                var nb_unites_metha = response[1];
                var gisements_table = response[2];
                var gisements_graphs = response[3];
                var pct_gaz_5km = response[4];
                var pct_gaz_5_10km = response[5];
                var iaa = response[6];
                var compostage = response[7];
                var alp = response[8];
				var ratios_perso = response[9]; 
				...
				$("#report-block").append('<button type="button" onclick="app.blocks.report.activate_report_col();" class="close" aria-label="Close"><span aria-hidden="true">&times;</span></button>');				
...
/*                         geoserver.lfCreateLayerWFS("Ressources Organiques Mobilisables", "cigale:rom_carte_comm_groupe_rom"+typename_suffix, app.blocks.carte_rom, true, [0,20], "NoTheme", {
                        ...
                        geoserver.lfCreateLayerWFS("Ressources Organiques Mobilisables", "cigale:rom_carte_comm_groupe_rom_rs"+typename_suffix, app.blocks.carte_rom, true, [0,20], "NoTheme", {
                            // fixed_url:"https://tmp.atmosud.org/api/geres.ratios.comm.php",
                            cql_filter: filter,
                            // cql_filter: filterOverlaps,
                            popup: false,
                            ...
                            zoom: true,            
							// NOTE: Pour l'instant on utilise le style actif mais qui ne correspond plus car on paut avoir modifié les rom 
                            // style_dict: app.blocks.mapview.layers["Ressources organiques mobilisables (hors STEP)"].multi_style[app.blocks.mapview.layers["Ressources organiques mobilisables (hors STEP)"].active_style].style, 
							style_dict: {
								jenks: {field: "Total potentiel rom (MWh)", njenks: 5, colorscale: [ "#ffffcc", "#a1dab4", "#41b6c4", "#2c7fb8", "#253494" ]}, // ESRI Blue 6
								pane: 'front',
				...
				$("#report-block").append("<p>* Calculé avec les ratios de mobilisation par défaut</p>");

                // Fin des cartes
                $("#report-block").append('</div>');   
                
				// Si pas d'unités de méthanisation alors "Aucune"
				if (nb_unites_metha.length == 0) {
					nb_unites_metha_display = "Aucune";
				} else {
					nb_unites_metha_display = nb_unites_metha.length
				};
				
                // Affichage des infos sur les unités de métanisations
                $("#report-block").append("<p><b>Unités de méthanisation sur le territoire</b></p>\
                <p>"+nb_unites_metha_display+" unité(s) de méthanisation présente(s) sur le territoire.</p>\
				...
				new Datablock("visualisation-graph-block-A", "#row-pies", true, 6, "", {
					graphique:{
						...
						title: "Gisement mobilisable (t/an)",
						tooltip:'{series.name} {point.percentage:.0f}% ({point.y}) t/an',
						option: {}
					}
				});	
				...
				new Datablock("visualisation-graph-block-B", "#row-pies", true, 6, "", {
					graphique:{
						...
						title: "Gisement mobilisable (MWh/an)",
						tooltip:'{series.name} {point.percentage:.0f}% ({point.y}) MWh/an',
						option: {}
					}
				});							

				// $("#report-block").append(create_html_table(gisements_graphs, ['groupe_rom','mob_t','mob_mwh'], ['groupe_rom','mob_t','mob_mwh']));
				$("#report-block").append(create_html_table(gisements_graphs, ['Type','Tonnes/an','Mwh/an'], ['Type','Tonnes/an','Mwh/an']));



				if (ratios_perso.length > 1) {
					$("#report-block").append("<p><b>Ci-dessous, les gisements mobilisables calculés à partir des ratios personnalisés</b></p></br>");
					$("#report-block").append(create_html_table(ratios_perso, ['Type','Tonnes/an','Mwh/an'], ['Type','Tonnes/an','Mwh/an']));
				};
                ...
                $("#report-block").append('<div class="row" id="row-plot-ddg">');
                    ...
                    // });  
                    geoserver.lfCreateLayerWFS("Surface agricole épandable</br>(ha/commune)", "cigale:ddg_carte_comm"+typename_suffix, app.blocks.carte_ddg, true, [0,20], "NoTheme", {
                        cql_filter: filter,
                        popup: false,
                        mouseover: false,
                        in_legend: true,
                        ...
                        style_dict: {
							...
							opacity: 0.8, fillOpacity: 0.5,
							jenks: {
								field: "Valorisation possible - ha", 
								njenks: 5,
								colorscale: ["#f7fbff","#c8ddf0","#73b3d8","#2879b9","#08306b"]
							},
							// classes: {
                    ...
                    // geoserver.lfCreateLayerWMS(
                        // "Parcelles agricoles épandables</br>selon contraintes environnementales et/ou géologiques", "cigale:ddg_classe_final", app.blocks.carte_ddg, true, [0,7], "NoTheme", {
                    ...
                    // *** Seconde carte DDG parcelles épandables
                    
                    app.blocks.carte_ddg_parc = new Datablock("carte-ddg-parc", "#row-plot-ddg", true, 6);  
                    app.blocks.carte_ddg_parc.createmap({
                        base_layers: [app.baseLayers.Esri_WorldTopoMap_fullzoom], 
                        draw: false, legend:true, zoom_to_layer:"territoire",
                        no_drag: true, no_zoom: true, attributions: false, title:"Parcelles épandables",
                        scale:true
                    });     

					// Le filtre spatial ne fonctionne pas encore sur WMS avec rayon personnalisé 
					if (app.territoire == "radius"){
						ddg_classes_dict =
						{
                            pane: "front", legend_carriage: true
                    ...
                    geoserver.lfCreateLayerWMS(
                        "Parcelles agricoles épandables</br>selon contraintes environnementales et/ou géologiques", "cigale:ddg_classe_final", app.blocks.carte_ddg_parc, true, [0,20], "NoTheme", 
						ddg_classes_dict
                    );                     
                                        
                    if (typename_suffix == ""){
                        geoserver.lfCreateLayerWFS("RapportTerritoireDdg", "cigale:entites", app.blocks.carte_ddg_parc, true, [0,20], "NoTheme", {
                            cql_filter: app.territoire_cql_filter,
                            json_object: false,
                            popup: false,
                            mouseover: false,
                        ...
                        L.circle([app.territoire_center[1], app.territoire_center[0]], 
                            app.blocks.mapview.layers["territoire"].layer.options
                        ).addTo(app.blocks.carte_ddg_parc.map);                             
                ...
                *Si une zone est à la fois concernée par des contraintes réglementaires et environnementales, elle est identifiée dans la catégorie contrainte réglementaire.</br>\
                ...
                // $("#report-block").append("\
				// <p>[X]% du territoire ne permet pas une implantation, d’un point de vue réglementaire.</p>\
                ...
                // ");
                $("#report-block").append("\
                <p>Les contraintes réglementaires concernent des distances aux habitations habitées par un tiers (50 m) et aux captages d’eau (35 m).</p>\
                <p>D’autres contraintes urbanistiques peuvent s’ajouter, qui ne sont pas prises en compte ici : zonages PLUs ou carte communale, Plan de Prévention des Risques. </p>\
                <p>La surface nécessaire varie en fonction de la typologie du projet et de sa puissance : entre 0,5 ha et 3 ha.</p>\
                ");
				...
				$("#report-block").append('<div id="stats-warning">* Les statistiques présentées le sont à titre indicatif et n\'engagent en aucun cas la responsabilité du GERES ou d\'AtmoSud</div>');
