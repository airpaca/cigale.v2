function Select(id, element_html, script, title, function_to_launch, header, dict_args) {
    ...
    */ 
    ...
    $('#' + this.id).on('change', function(e){  
        ...
        // window[app.get_territoire_app_function](e.target.value, e.target[$("#" + this.id).prop('selectedIndex')].dataset.tokens);
		app.territoire = e.target.value;
		app.territoire_token = e.target[$("#" + this.id).prop('selectedIndex')].dataset.tokens;
		console.log("LAUNCHING " + app.get_territoire_app_function);
