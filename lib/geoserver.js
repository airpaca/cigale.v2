...
function Geoserver(host_url){
    ...
    */
    ...
    this.getLegendWMS = function(typename, name, text, legend_carriage){
		...
		// FIXME: Pour les isolated workpsaces on a ajouté cigale/ avant wms? 
		// 	      En faire une option
        ...
        if(legend_carriage == true){
            ...
            html += '<img src="' + this.host + 'cigale/wms?REQUEST=GetLegendGraphic&VERSION=1.0.0&FORMAT=image/png&HEIGHT=15&LAYER='+typename+'&legend_options=fontColor:0x95a5a6;dx:5.2;dy:0.2;mx:0.2;my:0.2;&TRANSPARENT=true" alt="lgd" />';
        } else {
            // html += '<img src="' + this.host + 'wms?REQUEST=GetLegendGraphic&VERSION=1.0.0&FORMAT=image/png&HEIGHT=15&LAYER='+typename+'&legend_options=fontColor:0x95a5a6;dx:5.2;dy:0.2;mx:0.2;my:0.2;&TRANSPARENT=true" alt="lgd" />';
            html += '<img src="' + this.host + 'cigale/wms?REQUEST=GetLegendGraphic&VERSION=1.0.0&FORMAT=image/png&HEIGHT=15&LAYER='+typename+'&legend_options=fontColor:0x95a5a6;dx:5.2;dy:0.2;mx:0.2;my:0.2;&TRANSPARENT=true" alt="lgd" />';
            html += text;
        ...
        if (style_dict != false){
			...
			// FIXME: Comment avoir la même proportionalité de sympols que sur le carte ?                				
					...
					for (var i = 0; i < style_dict.jenks_calcules.bornes.slice(0,-1).length; i++) {
						...
						html += '<i style="background:' + style_dict.jenks_calcules.colors[i] + '"></i> ' + parseInt(from) + (parseInt(to) ? ' à ' + parseInt(to) : '+') ; 
						html += '</br>';						
					};
			...
			// FIXME: Comment avoir la même proportionalité de sympols que sur le carte ?                				
                ...
                if (style_dict.classes.param == "radius" ){
					...
					// Calcul des min max
					var min = style_dict.classes.grades[0];
					var max = style_dict.classes.grades[style_dict.classes.grades.length -1];
                    ...
                    if (typeof style_dict.classes.special != "undefined") {
                        ...
                        html += text; 
                        html += '<div style="line-height: 35px;"><i class="colorcircleBig" style="background:' + base_color + ';"></i><i class="colorcircleSmall" style="background:' + base_color + '; border: 1px solid white;"></i><text style="margin-left:50px;">De '+min[0]+' à '+max[0]+' '+ style_dict.classes.unite + '</text></div>';
                        // html += '<div><i class="colorcircle" style="background:' + base_color + '"></i>De '+min[0]+'</div>';
                        // html += '<div><i class="colorcircleBig" style="background:' + base_color + '"></i>à '+max[0]+'</div>';
                        html += "</br>";
...
/* 
            ...
            L.TileLayer.BetterWMS = L.TileLayer.WMS.extend({
                ...
                // -- Extension de la classe WMS de Leaflet.
                // Source et plus de possibilités: https://gist.github.com/rclark/6908938
                
                // - Permet une authentification simple et récupération des tuiles à travers un appel ajax 
                  // pour permettre d'accéder à des couches à accès restreint depuis Geoserver.
                ...
                createTile(coords, done) {
                    ...
                    $.ajax({
                        ...
                        beforeSend: function (xhr) {
                            // xhr.setRequestHeader ("Authorization", "Basic " + btoa(<?php echo '"'.$gs_lgn.'"';?> + ":" + <?php echo '"'.$gs_pwd.'"';?>));
                            xhr.setRequestHeader ("Authorization", "Basic " + btoa('admin:fs7nFRGjB'));
                        },           
...
/*     // *** Utilisation de la nouvelle fonction pour charger la couche ***
    wms_params = {
        name: "air:" + layer_name.split(".")[1].replace("_wms", ""), // "air:area_aenr_agri", // layer_name,
        layers: "air:" + layer_name.split(".")[1].replace("_wms", ""), // "air:area_aenr_agri",  // layers: layers,
        format: 'image/png',
        transparent: true,
        opacity: 0.5,
        attribution: "TEST",
    ...
    new_wms_layer = L.tileLayer.betterWms(url, wms_params);

    return new_wms_layer; */
        ...
        // var wms_layer = L.tileLayer.betterWms(           
    ...
    this.lfCreateLayerWFS = function (name, typename, map_block, add2map, zoom_levels, theme, dict_args) {
        ...
        if ( (dict_args != null) && (dict_args.special != null) ) {var special = dict_args.special;} else {var special = "";};  // Pour tout type d'infos à lier au layer pour l'application
		...
		// Si on veut passer une url spécifique 
		if (typeof(dict_args.fixed_url) != "undefined"){
			var url = dict_args.fixed_url;
			var the_data = dict_args.fixed_url_data;
		} else {
			var the_data = {};
		};
			...
			success: function(response,textStatus,jqXHR){
                ...
                if ( (jqXHR.style_dict != false) && (typeof jqXHR.style_dict.jenks != "undefined") ) {
					...
					for (style_index in map_block.layers[jqXHR.name].multi_style) {
						map_block.layers[jqXHR.name].multi_style[style_index].jenks_calcules = calc_jenks(
							response,
							map_block.layers[jqXHR.name].multi_style[style_index].style.jenks.field, 
							map_block.layers[jqXHR.name].multi_style[style_index].style.jenks.njenks,
							map_block.layers[jqXHR.name].multi_style[style_index].style.jenks.colorscale
						);
						map_block.layers[jqXHR.name].multi_style[style_index].style.jenks_calcules = map_block.layers[jqXHR.name].multi_style[style_index].jenks_calcules;
					};
                ...
                var wfs_layer = L.geoJson(response, {
                    ...
                    style: function (feature) {
                        ...
                        if ( (jqXHR.style_dict == false) && (("Polygon","MultiPolygon").indexOf(response.features[0].geometry.type) > -1)) {
                            ...
                            jqXHR.style_dict.fillColor = jqXHR.style_dict[jqXHR.style_dict.jenks.param];
                        ...
                        if (jqXHR.mouseover == true) {
                            layer.on({
								...
								click: function zoomToFeature(e){								
									...
									// Si besoin d'executer une fonction on cick
									// La fonction doit être définie directement dans dict_args.session_storage_click
									if (typeof(dict_args.session_storage_click) != "undefined") {
										dict_args.session_storage_click(e);
									};
    ...
    this.CreatelayerLink = function (name, typename, map_block, add2map, zoom_levels, theme, dict_args) {
        // Renvoie vers un autre site
