...
function createMap(){
    ...
    */
    ...
    $('#esri').change(function() {
        if(this.checked) {
            console.log("ESRI");
            map.removeLayer(app.baseLayer);
            app.baseLayer = Esri_WorldTopoMap;
            app.baseLayer.addTo(map);
            app.layers["cigale:dbg_grdf_reseau_5_10km"].layer_object.bringToFront();
            app.layers["cigale:dbg_grdf_reseau_5km"].layer_object.bringToFront();  
            app.layers["cigale:ddg_classe_final"].layer_object.bringToFront();
            app.layers["cigale:dbg_iaa"].layer_object.bringToFront(); 
            app.layers["cigale:rom_step"].layer_object.bringToFront(); 
            $('#steamen').prop("checked", false);
            $('#cartodb').prop("checked", false);        
        };  
    });    
    
    $('#steamen').change(function() {
        if(this.checked) {
            console.log("steamen");
            map.removeLayer(app.baseLayer);
            app.baseLayer = Stamen_Terrain;
            app.baseLayer.addTo(map);
            app.layers["cigale:dbg_grdf_reseau_5_10km"].layer_object.bringToFront();
            app.layers["cigale:dbg_grdf_reseau_5km"].layer_object.bringToFront();
            app.layers["cigale:ddg_classe_final"].layer_object.bringToFront(); 
            app.layers["cigale:dbg_iaa"].layer_object.bringToFront(); 
            app.layers["cigale:rom_step"].layer_object.bringToFront();             
            $('#esri').prop("checked", false);
            $('#cartodb').prop("checked", false);        
        };  
    });        
    
    $('#cartodb').change(function() {
        if(this.checked) {
            console.log("cartodb");
            map.removeLayer(app.baseLayer);
            app.baseLayer = CartoDB_Positron;
            app.baseLayer.addTo(map);
            app.layers["cigale:dbg_grdf_reseau_5_10km"].layer_object.bringToFront();
            app.layers["cigale:dbg_grdf_reseau_5km"].layer_object.bringToFront();  
            app.layers["cigale:ddg_classe_final"].layer_object.bringToFront(); 
    ...
    */
    L.easyPrint({
        title: 'Exporter la carte actuelle',
    ...
    map.on('zoomend', function () {
        ...
        */
        ...
        if (map.getZoom() < zoom_to_level) {
            ...
            // Si ortho checked on change basemap
            if ( $('#ortho').prop( "checked" ) ) {          
                console.log("Display Ortho");                 
    ...
    legend.onAdd = function (map) {
        ...
        div.innerHTML += '<img src="https://geoservices.atmosud.org/geoserver/wms?REQUEST=GetLegendGraphic&VERSION=1.0.0&FORMAT=image/png&HEIGHT=10&LAYER=mod_sudpaca_2017:mod_sudpaca_2017_no2_moyan&legend_options=fontColor:0x95a5a6;dx:5.2;dy:0.2;mx:0.2;my:0.2;&TRANSPARENT=true" alt="lgd" />';
